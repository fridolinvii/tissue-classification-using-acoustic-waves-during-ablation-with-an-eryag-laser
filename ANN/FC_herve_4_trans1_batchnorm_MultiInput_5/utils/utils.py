...
def convertToFrequency(data):
    Fs = 7.8125e6;  # Sampling frequency
    T = 1./Fs;             # Sampling period    
    L = data.shape[-1];    # Length of signal    
    t = np.linspace(0, T*L, L) # Time vector

    ha = np.expand_dims(np.hamming(L),axis=0)
    Y = fft.fft(data*ha);
    f = Fs*np.linspace(0,L/2,int(L/2))/L
    P2 = abs(Y/L);
    P1 = P2[:,1:int(L/2)+1]
