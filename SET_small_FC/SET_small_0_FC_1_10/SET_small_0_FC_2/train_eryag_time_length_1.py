...
from parameter import parameter_eryag_time_length_1 as param
...
#    viz = vis.Visdom(port=args.port)
    ...
    # create data manager
    data_manager = dm.DataManager_Time(args.path,args.traincsv_path, args.timerange, args.samplerate, args.frequencyrange,  args=None) 
    ...
    if args.model == "cnn_frequency":
        ...
        model = m.ConvClasi_time_length(data_manager.sequence_length(), num_classes=len(args.classes))
...
#    evaluater = Evaluater(args.eval_path, args, device=device)
    ...
    pathlib.Path('data/ErYAG/time_results_length_1').mkdir(parents=True, exist_ok=True)
    pathlib.Path('data/ErYAG/epoch_time_length_1').mkdir(parents=True, exist_ok=True)
 ...
 #                                    update='append')
        ...
        with th.no_grad():
            model = model.eval()
            data_manager_test = dm.DataManager_Time(args.path,args.testcsv_path, args.timerange, args.samplerate, args.frequencyrange,  args=None) 
...
if __name__ == "__main__":
    ...
    mod = 0
    if (mod==1):
        path = 'data/ErYAG/time_results_length_1/model_last.pt'
        state = th.load(path)
        args = state['args']
