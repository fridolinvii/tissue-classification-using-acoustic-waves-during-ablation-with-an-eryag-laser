...
#    evaluater = Evaluater(args.eval_path, args, device=device)
    ...
    pathlib.Path('data/ErYAG/frequency_results_length_1').mkdir(parents=True, exist_ok=True)
    pathlib.Path('data/ErYAG/epoch_frequency_length_1').mkdir(parents=True, exist_ok=True)


    
    alpha = 6
    for epoch in range(epoch_start,500): # args.num_epoch):
