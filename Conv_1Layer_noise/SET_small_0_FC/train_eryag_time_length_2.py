...
from parameter import parameter_eryag_time_length_2 as param
...
#    viz = vis.Visdom(port=args.port)
    ...
    # create data manager
    data_manager = dm.DataManager_Time_noise(args.path,args.traincsv_path, args.timerange, args.samplerate, args.frequencyrange, noise=True, args=None) 
    data_manager_test = dm.DataManager_Time_noise(args.path,args.testcsv_path, args.timerange, args.samplerate, args.frequencyrange,  noise=False, args=None) 

    # Parameters
    params = {'batch_size': args.batch_size,
              'shuffle': True,
              'num_workers': 5, #args.nb_workers,
              'pin_memory': True}
...
if __name__ == "__main__":
    ...
    mod = 1
    if (mod==1):
        path = 'data/ErYAG/time_results_length_2/model_best.pt'
        print("Loading...")
        state = th.load(path)
        args = state['args']
        print('Previous Epoch: '+str(state['epoch']))
