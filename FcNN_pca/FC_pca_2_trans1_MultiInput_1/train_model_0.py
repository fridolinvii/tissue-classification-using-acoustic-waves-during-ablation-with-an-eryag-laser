...
#    data_train = data_validate 
    ...
    in_data = dm.createPCADecomposition_precomputed(args=args, data1=data_train, data2=data_validate)
    pca = dm.createPCADecomposition(in_data=in_data, n_components=params["pca_components"])
    
    data_train = dm.createPCADecomposition_apply(args=args,data=data_train,pca=pca)
    data_validate = dm.createPCADecomposition_apply(args=args,data=data_validate,pca=pca)
    ...
    if model.error is True:
        ...
        for epoch in range(epoch_start, args.maxepoch):
            ...
            np.savetxt(args.path_to_folder + "/results/model_0/class_predict_last", confusion_matrix.astype(int), fmt='%d')
            ...
            if best_mean_loss < loss_value_mean_validate:
                ...
                utils.save_checkpoint(state, args.path_to_folder+'/results/model_0', "model_best.pt")
                np.savetxt(args.path_to_folder+'/results/model_0' + "/class_predict_best",confusion_matrix.astype(int), fmt='%d')

            utils.save_checkpoint(state, args.path_to_folder+'results/model_0', "model_last.pt")
...
if __name__ == "__main__":
    ...
    if input_args.mode == 'continue':
        ...
        path = args.path_to_folder+'/results/model_0/model_last.pt'
        state = th.load(path)
        args = state['args']
    ...
    pathlib.Path(args.path_to_folder+'/results/output_0').mkdir(parents=True, exist_ok=True)
    pathlib.Path(args.path_to_folder+'/results/model_0').mkdir(parents=True, exist_ok=True)

    ## write all the output in log file ##
    if input_args.logfile is True:
        time_string = time.strftime("%Y%m%d-%H%M%S")
        log_file_path = args.path_to_folder+'/results/output_0/output_{}.log'.format(time_string)
