...
from parameter import run_args_1 as run_args
...
#    data_train = data_validate 
    ...
    if model.error is True:
        ...
        for epoch in range(epoch_start, args.maxepoch):
            ...
            np.savetxt(args.path_to_folder + "/results/model_1/class_predict_last", confusion_matrix.astype(int), fmt='%d')
            ...
            if best_mean_loss < loss_value_mean_validate:
                ...
                utils.save_checkpoint(state, args.path_to_folder+'/results/model_1', "model_best.pt")
                np.savetxt(args.path_to_folder+'/results/model_1' + "/class_predict_best",confusion_matrix.astype(int), fmt='%d')

            utils.save_checkpoint(state, args.path_to_folder+'results/model_1', "model_last.pt")
...
if __name__ == "__main__":
    ...
    if input_args.mode == 'continue':
        ...
        path = args.path_to_folder+'/results/model_1/model_last.pt'
        state = th.load(path)
        args = state['args']
    ...
    pathlib.Path(args.path_to_folder+'/results/output_1').mkdir(parents=True, exist_ok=True)
    pathlib.Path(args.path_to_folder+'/results/model_1').mkdir(parents=True, exist_ok=True)

    ## write all the output in log file ##
    if input_args.logfile is True:
        time_string = time.strftime("%Y%m%d-%H%M%S")
        log_file_path = args.path_to_folder+'/results/output_1/output_{}.log'.format(time_string)
